...
class ramdiskmod():
    ...
    def unpack_kernel(self,path):
        ...
        lines=str(info,'utf-8').replace("\r","").split("\n")
        details={}
        for line in lines:
            if len(line.split("="))>0:
                key=line.split("=")[0]
    ...
    def repack_stuff(self,details):
        self.run(self.BOOTIMG + " mkinitfs " + self.RAMDISK + " | " + self.BB + " gzip -c > "+self.TARGET+".cpio.gz")
        cmd=self.BOOTIMG+" mkimg --kernel "+os.path.join(self.RPATH,"kernel")+" --ramdisk "+self.TARGET+".cpio.gz"+" "
        cmd += "--base 0x0 "
        ...
        if "BOARD_SECOND_OFFSET" in details:
            cmd+="--second_offset 0x"+details["BOARD_SECOND_OFFSET"]+" "
        if "BOARD_KERNEL_CMDLINE" in details:
            cmd+="--cmdline \""+details["BOARD_KERNEL_CMDLINE"]+"\" "
        if "BOARD_OS_VERSION" in details:
            cmd+="--osversion \""+details["BOARD_OS_VERSION"]+"\" "
        if os.path.exists(os.path.join(self.RPATH,"dtb")):
            cmd+="--dt "+os.path.join(self.RPATH,"dtb")+" "
        cmd+="-o "+self.TARGET
        self.run(cmd)
        os.remove(self.TARGET+".cpio.gz")
        self.TARGET=self.TARGET.replace("\\","/")
        path=self.TARGET[:self.TARGET.rfind("/")]+"/tmp"
        self.rmrf(path)
    ...
    def patch_stuff(self,TMPPATH,BOOTPATH):
        ...
        if not os.path.exists(os.path.join(self.RAMDISK,"sepolicy@0644")):
            ...
            ext4 = Ext4Extract()
            #simg = Simg2Img()
            ext4.extractext4(os.path.join(BOOTPATH, "vendor.img"), "/etc/selinux/precompiled_sepolicy",os.path.join(TMPPATH, "precompiled_sepolicy"))
            if os.path.exists(os.path.join(TMPPATH,"precompiled_sepolicy")):
                print("- Copying precompiled_sepolicy, as sepolicy file is missing in boot !")
                shutil.copyfile(os.path.join(TMPPATH,"precompiled_sepolicy"),os.path.join(self.RAMDISK,"sepolicy@0644"))        #$BOOTIMG magiskpolicy --load $RAMDISK/sepolicy@0644 --save $RAMDISK/sepolicy@0644 --minimal
            else:
    ...
    def sign(self,keyname,avbversion,outfilename,vbmetatarget):
        ...
        if avbversion==1:
            ...
            if ".pk8" in keyname:
                ...
                if os.path.exists(self.TARGET + ".signed"):
                    os.remove(self.TARGET + ".signed")
            ...
            avb=Avb()
            try:
                avb.add_hash_footer(self.TARGET,partition_size,vbmetatarget,'sha256',salt,None,'SHA256_RSA4096',name+".pem",None,0,0,None,None,None,None,include_descriptors_from_image,None,None,None,None,None,output_vbmeta_image,False,False,False,False)
            except:
                avb.add_hash_footer(self.TARGET, partition_size, vbmetatarget, 'sha256', salt, None, 'SHA256_RSA2048', name + ".pem", None, 0, 0, None, None, None, None, include_descriptors_from_image, None, None, None, None, None, output_vbmeta_image, False, False, False, False)
    ...
    def go(self,args,TMPPATH,BOOTPATH,param):
        ...
        vbmetatarget=args.vbmetatarget
        avbversion=0
        modulus=""
        with open(self.BOOTIMAGE,'rb') as rf:
            ...
            if sig==b"\x30\x82":
                ...
                if os.path.exists(vbmetaname):
                    modinfo=get_vbmeta_pubkey(vbmetaname,vbmetatarget)
                    if modinfo==None:
        ...
        if not args.nopatch==True:
            self.patch_stuff(TMPPATH,BOOTPATH)
        if (self.custom==True):
        ...
        if os.path.exists("key"):
            ...
            if keyname != None:
                self.sign(keyname,avbversion,self.TARGET + ".signed",vbmetatarget)
            else:
...
def main():
    ...
    parser.add_argument('-vbmetatarget', '-vt', dest='vbmetatarget', action='store', default='boot', help='vbmeta partition')
    ...
    if args.justunpack==True:
        ...
        rdm.go(args,TMPPATH,BOOTPATH,param)
